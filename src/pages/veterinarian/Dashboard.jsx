...
const VeterinarianDashboard = () => {
  ...
  const todayAppointments = useMemo(() => myAppointments.filter(a => (a.datetime || a.date)?.startsWith(todayStr)), [myAppointments, todayStr]);
  const pendingCount = useMemo(() => myAppointments.filter(a => a.status === 'PENDIENTE').length, [myAppointments]);
  const confirmedCount = useMemo(() => myAppointments.filter(a => a.status === 'CONFIRMADA').length, [myAppointments]);
  const completedCount = useMemo(() => myAppointments.filter(a => a.status === 'COMPLETADA').length, [myAppointments]);
